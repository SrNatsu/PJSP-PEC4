...
describe('Board', () => {
        ...
        const expectedBoard = board.board.map((row, rowIndex) =>
            row.map((cell, colIndex) => {
                ...
                return board.board
                    ...
                    .flatMap(r => r.slice(Math.max(0, colIndex - 1), Math.min(board.size, colIndex + 2)))
    ...
    test('Should fill the board with valid values after generating', () => {
        board.generate();
        const isValid = board.board.flat().every(cell => cell === -1 || (cell >= 0 && cell <= 8));
...
describe('Game', () => {
    let mockConfigInstance, mockBoardInstance, mockGameLogicInstance, mockUIInstance;
    ...
    test('Should instantiate required classes when generate button is clicked', () => {
        ...
        const boardSpy = jest.spyOn(Board.prototype, 'generate').mockImplementation(() => {});
        const uiSpy = jest.spyOn(UI.prototype, 'renderBoard').mockImplementation(() => {});
